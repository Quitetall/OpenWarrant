...
- **scope:** capture_sources and resolve_refs with an explicit filesystem shell; F03, T11, T12, T13, T14, T15.
- **evidence:** T11-T15 prove exact unit lookup after heading changes, original-byte pins, opaque fixture capture and enumeration-independent lock bytes.

### OBL-002 — the controls reject their stated counterexamples
- **scope:** the refusal cases within capture_sources and resolve_refs with an explicit filesystem shell.
- **evidence:** Missing/ambiguous identity, digest mismatch, traversal, absolute paths, symlinks and changed-during-capture sources refuse publication while preserving previous output.
...
```bash
cargo run -p openwarrant-compiler --example rc2_probe -- --feature F03 --fixtures conformance/rc2
```
