...
- `crates/openwarrant-cli/src/context.rs`
- `crates/openwarrant-compiler/src/lib.rs`
- `conformance/rc2/cli-context/`
...
Exercise the documented command sequence from fresh inputs to a checked offline package. JSON mode emits one report on stdout with diagnostics on stderr; repeated requests reproduce bytes. Existing war compile keeps its legacy behavior.

Malformed requests, missing context, access denial, budget overflow and write failures exit nonzero without partial publication or hidden prompts. Unknown cost or readiness must not appear as zero or granted permission.
