...
def main():
    ...
    profile_run = run(["cargo", "+1.97.1", "run", "--locked", "--quiet", "-p",
                       "lamu-openwarrant", "--example", "budget_profile"], root)
    if profile_run["exit_code"]:
        raise RuntimeError(profile_run["stderr"])
    profile = json.loads(profile_run["stdout"])
    if profile["id"] != "lamu.openwarrant.budgets/1" or profile["sdk_revision"] != expected_sdk:
    ...
    tests = run(["cargo", "+1.97.1", "test", "--locked", "-p", "lamu-openwarrant",
                 "--test", "budget", "--", "--test-threads=1"], root)
    expected_tests = [
        "budget_counts_final_framing_and_inclusive_limit",
        "cache_tracks_full_basis_and_retains_previous_results",
        "newly_applicable_rule_invalidates_cached_view",
    ]
    ...
    receipt = {
        ...
        "scope": "T38-T41 final entry budget, immutable cached views and complete basis invalidation",
