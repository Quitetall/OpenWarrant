...
- **scope:** Fresh installation and documented compiler commands on Linux and macOS; P2-INSTALL.
- **evidence:** Candidate packaging includes executable, license, versioned standard/format/schema/fixture manifest and checksums. Fresh Linux/macOS installs execute the documented examples; help and JSON reports agree with the installed build.

### OBL-002 — the controls reject their stated counterexamples
- **scope:** the refusal cases within Fresh installation and documented compiler commands on Linux and macOS.
- **evidence:** Wrong-platform assets, checksum mismatch, missing normative companions and stale installed binaries fail the installation/release checks. Signed README or other pinned deliveries change only through the approved correction/successor path.
