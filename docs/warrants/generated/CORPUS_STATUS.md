...
- Revision 1.0.0 is ACCEPTED (§101.2) and normative. A Warrant compiles against the revision its authorization recorded (§14), so a later revision does not move an authorized contract: 57 against 0.1.0-draft.1, 8 against 0.1.0-draft.3, 25 against 1.0.0.
...
| *unassigned — declares no [[roadmap]]* | — | not derivable — a Warrant naming no phase belongs to no Objective | 0 | 23 | 0 | 0 | 0 |
...
|---|---|
| artifact digests verify | 36 |
| required deliverables exist | 36 |
| every required gate has admissible result | 34 |
| no blocker remains | 33 |
| no required unknown remains | 33 |
| every required obligation is dispositioned | 31 |
| independence requirements are met | 31 |
...
## Warrants (90) — invalid 0 · draft 42 · ready_to_resolve 19 · would_satisfy 0 · resolved **29**
