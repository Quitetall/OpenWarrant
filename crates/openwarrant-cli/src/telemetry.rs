...
mod tests {
    ...
    /// OBL-002's actual acceptance test, as a unit test over the type: a measure
    /// that was never taken must not serialize as a number.
    #[test]
    fn an_untaken_measure_never_serializes_as_zero() {
        let m = Measure::not_yet("nothing instruments this");
        let json = serde_json::to_string(&m).expect("serializes");
        assert!(json.contains("not_measurable_yet"), "{json}");
        assert!(
            !json.contains(":0") && !json.contains("\"value\""),
            "an untaken measure must not carry a value at all: {json}"
        );
    }

    /// A measured zero is a different claim from an unmeasured one, and both
    /// must be expressible. `auto-authorizable fraction` is genuinely zero.
    #[test]
    fn a_measured_zero_is_distinguishable_from_an_unmeasured_one() {
        let taken = Measure::taken(0, "counted");
        let untaken = Measure::not_yet("no method");
        assert_ne!(taken, untaken);
        let t = serde_json::to_string(&taken).expect("serializes");
        assert!(t.contains("\"value\":0"), "{t}");
    }

    /// §94 says eighteen and §100 says sixteen. If either constant drifts from
    /// the specification, the baseline silently covers a different set.
    #[test]
    fn the_measure_and_metric_counts_match_the_sas() {
        assert_eq!(TELEMETRY_MEASURES.len(), 18);
        assert_eq!(DERIVED_METRICS.len(), 8);
        assert_eq!(
            SUCCESS_METRICS.len(),
            16,
            "§100: nine reduced + seven increased"
        );
    }

    /// Every §100 metric is a baseline or `no baseline` — never a delta. One
    /// measurement cannot support a direction of travel.
    #[test]
    fn no_success_metric_claims_a_direction() {
        for m in SUCCESS_METRICS {
            assert!(!m.contains("reduced") && !m.contains("increased"), "{m}");
        }
    }
}
